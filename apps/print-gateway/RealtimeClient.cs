...
public sealed class RealtimeClient(GatewayConfiguration configuration)
{
    public async Task ListenAsync(string clientId, Func<Task> refreshInventory, CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                await ReceiveAsync(socket, refreshInventory, cancellationToken);
            }
    ...
    private static async Task ReceiveAsync(ClientWebSocket socket, Func<Task> refreshInventory, CancellationToken cancellationToken)
    {
        ...
        while (socket.State == WebSocketState.Open)
        {
            ...
            try
            {
                ...
                if (json.RootElement.TryGetProperty("event", out var eventName) && eventName.GetString() == "broadcast" && json.RootElement.TryGetProperty("payload", out var payload) && payload.TryGetProperty("payload", out var body) && body.TryGetProperty("type", out var type) && type.GetString() == "REFRESH_PRINTER_INVENTORY" && body.TryGetProperty("version", out var version) && version.GetInt32() == 1)
                    await refreshInventory();
            }
            catch (JsonException) { }
            message.SetLength(0);
