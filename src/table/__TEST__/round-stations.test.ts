...
test("copies server product stations into a mixed round and only changes subsequent rounds", async () => {
  vi.mocked(findActiveSession).mockResolvedValue({
    success: true,
    data: { status: "OPEN", orderId: "order-1", currentRound: 0 },
  } as never);
  const products = ["KITCHEN", "BAR", null].map((preparationStation, id) => ({
    id: String(id),
    price: 10,
    name: "Producto",
    preparationStation,
  ...
  vi.mocked(addOrderItems).mockResolvedValue({
    success: true,
    data: undefined,
  });
  ...
  expect(
    vi.mocked(addOrderItems).mock.calls[0][2].map((i) => i.preparationStation),
  ).toEqual(["KITCHEN", "BAR", null]);
  products[0].preparationStation = "BAR";
  await addRound("table-1", "company-1", items);
  expect(vi.mocked(addOrderItems).mock.calls[0][2][0].preparationStation).toBe(
    "KITCHEN",
  );
  expect(vi.mocked(addOrderItems).mock.calls[1][2][0].preparationStation).toBe(
    "BAR",
  );
});
