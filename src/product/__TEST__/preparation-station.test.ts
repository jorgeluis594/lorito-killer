...
import { SingleProductSchema, PackageProductSchema, ServiceProductSchema } from "../schema";
...
const product: ProductService = {
  id: "product-1", companyId: "company-1", type: "ServiceProduct",
  name: "Producto", price: 10, description: "", categories: [], hidden: false,
};

test.each([SingleProductSchema, PackageProductSchema, ServiceProductSchema])(
  "validates stations on every product schema", (schema) => {
    ...
    expect(schema.safeParse({ ...input, preparationStation: "OTHER" }).success).toBe(false);
  },
);
...
test("updates a station, explicitly clears it, and preserves it when omitted", async () => {
  ...
  for (const preparationStation of ["BAR", null, undefined] as const) {
    expect((await update({ ...product, preparationStation })).success).toBe(true);
...
test("rejects invalid station updates before accessing the database", async () => {
  const write = vi.fn();
  setPrismaClient({ product: { update: write } } as never);
  expect((await update({ ...product, preparationStation: "OTHER" } as never)).success).toBe(false);
